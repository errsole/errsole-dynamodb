...
const { DynamoDBDocumentClient, PutCommand, GetCommand, ScanCommand, DeleteCommand, QueryCommand, UpdateCommand, BatchWriteCommand } = require('@aws-sdk/lib-dynamodb');
...
class ErrsoleDynamoDB extends EventEmitter {
  ...
  async deleteExpiredLogs () {
    ...
    try {
      ...
      const configResult = await this.getConfig('logsTTL');
      if (configResult.item) {
        const parsedTTL = parseInt(configResult.item.value, 10); // Parse TTL value from the config
        logsTTL = isNaN(parsedTTL) ? DEFAULT_LOGS_TTL : parsedTTL * 1000; // Convert seconds to milliseconds
      }
      ...
      do {
        ...
        if (itemsToDelete.length > 0) {
          ...
          const batchDeleteParams = {
            RequestItems: {
              [this.logsTable]: deleteRequests
            }
          };

          // Execute batch delete
          await this.documentClient.send(new BatchWriteCommand(batchDeleteParams));
          batchDeleteCount += itemsToDelete.length;
        }

        ExclusiveStartKey = result.LastEvaluatedKey;

        // Pause for 10 seconds between batches to prevent rate limiting
        if (ExclusiveStartKey) {
          await new Promise((resolve) => setTimeout(resolve, 10000));
        }
      } while (ExclusiveStartKey);

      console.log(`Finished deleting expired logs. Total deleted: ${batchDeleteCount}`);
    } catch (err) {
      console.error('Error deleting expired logs:', err);
    } finally {
