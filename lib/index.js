...
const { DynamoDBClient, CreateTableCommand, waitUntilTableExists, UpdateTimeToLiveCommand } = require('@aws-sdk/client-dynamodb');
...
class ErrsoleDynamoDB extends EventEmitter {
  ...
  async setLogsTTL () {
    ...
    this.logsTTL = configResult.item.value;
  }
  ...
  async enableTTLForLogsTable () {
    ...
    try {
      await this.dynamoDBClient.send(new UpdateTimeToLiveCommand(params));
    } catch (error) {
  ...
  async ensureLogsTTL () {
    const DEFAULT_LOGS_TTL = 30 * 24 * 60 * 60;
    const configResult = await this.getConfig('logsTTL');
