...
const { DynamoDBClient, CreateTableCommand, waitUntilTableExists, UpdateTimeToLiveCommand, DescribeTimeToLiveCommand } = require('@aws-sdk/client-dynamodb');
...
class ErrsoleDynamoDB extends EventEmitter {
  constructor (options = {}) {
    ...
    const { region, accessKeyId, secretAccessKey, tablePrefix } = options.region;
    ...
    const basePrefix = 'errsole';
    const customPrefix = tablePrefix ? tablePrefix.toLowerCase().replace(/[^a-zA-Z0-9]/g, '') : '';
    this.tablePrefix = customPrefix ? `${basePrefix}_${customPrefix}` : basePrefix;
  ...
  async init () {
    ...
    await this.enableTTLForLogsTable();
    await this.enableTTLForNotificationsTable();
    this.emit('ready');
  ...
  async setLogsTTL () {
    ...
    if (configResult.item && configResult.item.value) {
      this.logsTTL = Math.floor(configResult.item.value / 1000);
    } else {
      this.logsTTL = null;
    }
  }

  async enableTTLForLogsTable () {
    try {
      const ttlStatus = await this.dynamoDBClient.send(
        new DescribeTimeToLiveCommand({ TableName: this.logsTable })
      );
      ...
      console.error(`Failed to enable TTL on table ${this.logsTable}:`, error);
      throw error;
    }
  }

  async enableTTLForNotificationsTable () {
    try {
      const ttlStatus = await this.dynamoDBClient.send(
        new DescribeTimeToLiveCommand({ TableName: this.notificationsTable })
      );
      ...
      console.error(`Failed to enable TTL on table ${this.notificationsTable}:`, error);
      throw error;
    }
  }
  ...
  async getConfig (key) {
    ...
    if (item && key === 'logsTTL') {
      item.value = parseInt(item.value, 10) * 1000;
    }
    return { item };
  }
    ...
    if (key === 'logsTTL') {
      if (typeof value === 'number') {
        ...
        if (!isNaN(numericValue)) {
          value = Math.floor(numericValue / 1000);
        } else {
          throw new Error('Invalid logsTTL value. It must be a number.');
        }
      }
    }
  ...
  async insertNotificationItem (notification = {}) {
    const hashedMessage = String(notification.hashed_message);
    const errsoleId = Number(notification.errsole_id);

    const now = new Date();
    const expireAt = Math.floor((now.getTime() + this.logsTTL) / 1000); // epoch in seconds
    const newItem = {
      hashed_message: hashedMessage, // Hash Key
      errsole_id: errsoleId,
      created_at: now.toISOString(), // Range Key
      updated_at: now.toISOString(),
      expire_at: expireAt
    };

    try {
      // Fetch the previous notification item (if any)
      const { Items: previousItems = [] } = await this.documentClient.send(new QueryCommand({
        TableName: this.notificationsTable,
        KeyConditionExpression: 'hashed_message = :hashedMessage',
        ExpressionAttributeValues: { ':hashedMessage': hashedMessage },
        ScanIndexForward: false,
        Limit: 1
      }));
      const previousNotificationItem = previousItems[0] || null;

      // Insert the new notification item
      await this.documentClient.send(new PutCommand({
        TableName: this.notificationsTable,
        Item: newItem
      }));

      // Prepare start and end timestamps for today in UTC
      const startOfDayUTC = new Date();
      startOfDayUTC.setUTCHours(0, 0, 0, 0);
      const endOfDayUTC = new Date();
      endOfDayUTC.setUTCHours(23, 59, 59, 999);

      // Count notifications created today with the same hash.
      const { Count: itemCount = 0 } = await this.documentClient.send(new QueryCommand({
        TableName: this.notificationsTable,
        KeyConditionExpression: 'hashed_message = :hashedMessage AND created_at BETWEEN :startTime AND :endTime',
        ExpressionAttributeValues: {
          ':hashedMessage': hashedMessage,
          ':startTime': startOfDayUTC.toISOString(),
          ':endTime': endOfDayUTC.toISOString()
        },
        Select: 'COUNT'
      }));
      const todayNotificationCount = itemCount;
